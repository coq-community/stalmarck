...
intros Ar LL H' S H'0 H'1; repeat (split; auto).
exists S; split; auto.
...
apply doTripletsTrans with (S2 := S') (t := t); auto.
...
intros H'8; inversion H'8; red in |- *; auto.
...
intros H'4.
generalize (addEqMemCorrect Ar a (rZComp b) S H'2 H'3); auto.
case (addEqMem Ar a (rZComp b)); auto.
intros Ar2 b2 L2; case b2.
intros H'5; exists S; split; auto.
red in |- *.
exists a.
apply eqStateRzTrans with (b := rZComp (rZComp b)); auto.
apply eqStateRzInv; auto.
...
case (evalTraceF t0 Ar2); auto.
intros Ar2' b2' L2'; case b2'; simpl in |- *; auto.
...
case H'4.
intros x H'13.
apply eqStateRzContr with (a := x); auto.
...
rewrite H'13.
apply H'8; auto.
Contradict H'15; auto.
cut (InclEq _ eqRz L2 (appendf _ rZlt eqRz rZltEDec L2 L2')).
intros H'16; inversion H'16; red in |- *; auto.
apply appendfInclEq1; auto.
Contradict H'15; auto.
cut (InclEq _ eqRz L2' (appendf _ rZlt eqRz rZltEDec L2 L2')).
intros H'16; inversion H'16; red in |- *; auto.
...
intros Ar1' b1' L1'; case b1'; simpl in |- *; auto.
intros H'10; Elimc H'10; intros S' E; Elimc E; intros H'10 H'11.
exists S'; split; auto.
apply
 stalmarckPSplit
...
intros H'15; inversion H'15; red in |- *; auto.
apply appendfInclEq1; auto.
Contradict H'14; auto.
cut (InclEq _ eqRz L1' (appendf _ rZlt eqRz rZltEDec L1 L1')).
intros H'15; inversion H'15; red in |- *; auto.
...
exists S'; split; auto.
apply stalmarckPSplit with (a := a) (b := b) (S2 := S') (S3 := S'0); auto.
split; auto.
red in |- *.
intros i j H'17.
apply interMemEqStateRz; auto.
...
rewrite H'18.
apply H'12; auto.
Contradict H'20; auto.
cut (InclEq _ eqRz L2 (appendf _ rZlt eqRz rZltEDec L2 L2')).
intros H'21; inversion H'21; red in |- *; auto.
apply appendfInclEq1; auto.
Contradict H'20; auto.
cut (InclEq _ eqRz L2' (appendf _ rZlt eqRz rZltEDec L2 L2')).
intros H'21; inversion H'21; red in |- *; auto.
...
2: unfold appendRz in |- *; red in |- *; apply appendfOlist; auto.
...
2: rewrite H'16; auto.
2: apply H'7; auto.
2: Contradict H'17; auto.
2: cut (InclEq _ eqRz L1 (appendf _ rZlt eqRz rZltEDec L1 L1')).
2: intros H'18; inversion H'18; red in |- *; auto.
2: apply appendfInclEq1; auto.
2: Contradict H'17; auto.
2: cut (InclEq _ eqRz L1' (appendf _ rZlt eqRz rZltEDec L1 L1')).
2: intros H'18; inversion H'18; red in |- *; auto.
2: apply appendfInclEq2; auto.
generalize (H'0 Ar2 LL H'8 _ H'9 H'10).
case (evalTraceF t0 Ar2).
intros Ar2' b2' L2'; case b2'; simpl in |- *; auto.
...
exists S'0; split; auto.
apply stalmarckPSplit with (a := a) (b := b) (S2 := S'0) (S3 := S'); auto.
split; auto.
red in |- *.
intros i j H'20.
apply interMemEqStateRz; auto.
...
2: rewrite H'20; auto.
2: apply H'12; auto.
2: Contradict H'21; auto.
2: cut (InclEq _ eqRz L2 (appendf _ rZlt eqRz rZltEDec L2 L2')).
2: intros H'22; inversion H'22; red in |- *; auto.
2: apply appendfInclEq1; auto.
2: Contradict H'21; auto.
2: cut (InclEq _ eqRz L2' (appendf _ rZlt eqRz rZltEDec L2 L2')).
2: intros H'22; inversion H'22; red in |- *; auto.
...
repeat (split; auto).
exists (interState S' S'0); split; auto.
apply stalmarckPSplit with (a := a) (b := b) (S2 := S') (S3 := S'0); auto.
apply inclStateTrans with (addEq (a, rZComp b) S); auto.
apply stalmarckIncl with (L := LL); auto.
apply inclStateTrans with (addEq (a, b) S); auto.
apply stalmarckIncl with (L := LL); auto.
Qed.
(* A computable InDec *)
...
intros H'0; left; rewrite H'0; auto with datatypes.
intros H'0; case (InDec A H' a l0).
intros H'1; left; auto with datatypes.
intros H'1; right; simpl in |- *; red in |- *; intros H'2; case H'2; auto.
Defined.
...
intros LL f T H'; elim T; simpl in |- *; auto.
intros t; case t; auto.
intros r r0 r1 r2; case (InDec _ tripletDec (Triplet r r0 r1 r2) (f r0));
 auto.
intros H'0 H'1; apply (H' r0); auto.
intros; discriminate.
intros t; case (fInT f t); auto.
intros; discriminate.
intros a b t; case (fInT f t); auto.
intros; discriminate.
Qed.
(* Build the hashtable *)
...
intros r; case r; simpl in |- *; auto with datatypes.
intros r; case r; simpl in |- *; auto with datatypes.
intros t; case t; auto.
...
repeat rewrite Eq3.
repeat rewrite rArrayDef1 with (m := valRz a); auto with datatypes.
repeat rewrite rArrayDef2 with (m2 := valRz a); auto with datatypes.
rewrite rArrayDef2 with (m2 := valRz a); auto.
case (rNatDec (valRz r0) (valRz a)); intros Eq3.
repeat rewrite Eq3.
repeat rewrite rArrayDef1 with (m := valRz a); auto with datatypes.
rewrite rArrayDef2 with (m2 := valRz a); auto with datatypes.
rewrite rArrayDef2 with (m2 := valRz a); auto with datatypes.
case (rNatDec (valRz r1) (valRz a)); intros Eq2.
repeat rewrite Eq2.
repeat rewrite rArrayDef1 with (m := valRz a).
...
apply pointerDecreaseDef; simpl in |- *; auto.
intros r; case r; simpl in |- *; intros; discriminate.
apply pointToClassRefDef; simpl in |- *; auto.
intros r; case r; simpl in |- *; intros; discriminate.
apply pointToClassClassRef; simpl in |- *; auto.
...
intros H'0; inversion H'0.
intros r; case r; simpl in |- *; intros; discriminate.
apply OlistArrayDef; simpl in |- *; auto.
intros r; case r; simpl in |- *.
intros H' Lr H'0; inversion H'0; red in |- *; apply OlistNil; auto.
intros H' Lr H'0; inversion H'0; red in |- *; apply OlistNil; auto.
intros Lr H'; inversion H'; red in |- *; apply OlistNil; auto.
Qed.
...
intros l r r0.
CaseEq (fInT (getT (buildL l)) T); auto.
CaseEq (addEqMem (rArrayInit vM (fun _ : rNat => class nil)) r rZFalse).
intros r1 b; case b; auto.
intros l0 H' H'0 H'1.
case (TautoRTauto e).
intros H'3 H'4; apply H'4; auto.
case (rTautotTauto (norm e)).
intros H'5 H'6; apply H'6; auto.
red in |- *; rewrite H'1.
apply stalmarckGivesValidEquation with (S := addEq (r, rZFalse) nil); auto.
generalize
 (addEqMemCorrect (rArrayInit vM (fun _ : rNat => class nil)) r rZFalse nil).
rewrite H'; auto.
intros H'2; apply H'2; auto.
apply rIwF; auto.
exact initCorrect; auto.
CaseEq (evalTraceF T r1).
intros r2 b0; case b0; auto.
...
apply fInTCorrect with (f := getT (buildL l)); auto.
intros n; apply getTCorrect; auto.
apply rIwF; auto.
exact initCorrect; auto.
Qed.

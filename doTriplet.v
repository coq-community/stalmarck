...
intros S1 S2 S3 t H' H'1; inversion H';
 apply doTripletEqAux2 || apply doTripletEqAux1; auto;
 generalize (eqStateEq _ _ _ _ H'1 H); auto.
Qed.
...
intros p q S1 t H'; exists ((p, q) :: nil); auto.
Qed.
...
intros p q r s S1 t H'; exists ((p, q) :: (r, s) :: nil); auto.
Qed.
...
intros S1 S2 t H'; inversion H';
 apply doTripletUnionEx2 with (t := t) ||
   apply doTripletUnionEx1 with (t := t); auto; rewrite H1; 
 auto.
Qed.
...
apply eqStateTrans with (unionState (unionState ((p, q) :: nil) S3) S1); auto.
apply eqStateTrans with (unionState (unionState S3 ((p, q) :: nil)) S1); auto.
apply eqStateSym; auto.
replace (addEq (p, q) S1) with (unionState ((p, q) :: nil) S1); auto.
...
intros S1 S2 S3 t H'; inversion H';
 ...
 auto; generalize (eqStateIncl _ _ _ _ (unionStateInclR S3 S1) H); 
 auto.
Qed.
(* It is monotone *)
...
intros f S1 S2 t H' H'0; elim H'0; simpl in |- *; auto;
 intros p q r H'1 H'2 H'3;
 apply realizeStateEvalAux1 with (1 := H'1) ||
   apply realizeStateEvalAux2 with (1 := H'1); auto;
 ...
 simpl in |- *; auto; try repeat rewrite H'3; auto.
Qed.
...
intros f S1 S2 t H'; elim H'; intros p q r H'1 H2 H'3;
 ...
 simpl in |- *; case (rZEval f r); simpl in |- *; auto;
...
intros S1 S2 S3 t H' H'0.
red in |- *; split; apply doTripletBisIncl with (S1 := S1) (t := t); auto.
Qed.
...
generalize (eqStateIncl (addEq (p, q) S1)); intros Eq1; red in |- *; auto 8.
Qed.
...
intros t S1 S2 S3 H' H'0; (generalize (eqStateIncl S1); intros Eq0);
 ...
 | apply inclStateTrans with (S2 := S2); auto;
    apply doTripletIncl with (t := t); auto ]; generalize H'0; 
 clear H'0; elim H'; intros p q r H'0 H'1 H'2;
 apply doTripletInvolExAux2 with (1 := H'1) ||
   apply doTripletInvolExAux1 with (1 := H'1); auto.
Qed.

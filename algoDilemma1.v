...
intros t1; case t1; simpl in |- *; auto.
intros t1'; case t1'; simpl in |- *; auto.
...
apply dTCorrect with (S2 := S') (S3 := S'0); auto.
split; auto.
red in |- *.
intros i j H'0.
...
exists S'1; repeat (split; auto).
apply stalmarckPSplit with (a := a) (b := b) (S2 := S') (S3 := S'1); auto.
apply dTCorrect with (S2 := S') (S3 := S'1); auto.
split; auto.
red in |- *.
intros i j H'1.
apply interMemEqStateRz; auto.
...
intros H'1; inversion H'1; red in |- *; auto.
...
exists S'1; repeat (split; auto).
apply stalmarckPSplit with (a := a) (b := b) (S2 := S'1) (S3 := S'); auto.
apply dTCorrect with (S2 := S'1) (S3 := S'); auto.
split; auto.
red in |- *.
intros i j H'1.
apply interMemEqStateRz; auto.
...
apply H'17; auto.
Contradict H'0; auto.
cut (InclEq _ eqRz L1 (appendf _ rZlt eqRz rZltEDec L1 L1')); auto.
intros H'1; inversion H'1; red in |- *; auto.
apply appendfInclEq1; auto.
Contradict H'0; auto.
cut (InclEq _ eqRz L1' (appendf _ rZlt eqRz rZltEDec L1 L1')); auto.
...
intros Ar' L' H'15; elim H'15;
 ...
 | clear H'15 ]; auto.
case L'; auto.
intros H'15 H'21; repeat (split; auto).
exists S; split; [ idtac | split ]; auto.
apply rArrayStateGet with (Ar := Ar); auto.
intros e; apply H'15; auto.
red in |- *; intros H'26; inversion H'26.
intros r l H'15 H'21; repeat (split; auto).
exists (interState S'0 S'); split; [ idtac | split ]; auto.
apply stalmarckPSplit with (a := a) (b := b) (S2 := S'0) (S3 := S'); auto.
apply dTCorrect with (S2 := S'0) (S3 := S'); auto.
apply inclStateTrans with (addEq (pair a b) S); auto.
apply stalmarckIncl with (L := LL); auto.
apply inclStateTrans with (addEq (pair a (rZComp b)) S); auto.
apply stalmarckIncl with (L := LL); auto.
intros e H'15; rewrite H'24; auto.
apply H'17; auto.
Contradict H'15.
cut (InclEq _ eqRz L1 (appendf _ rZlt eqRz rZltEDec L1 L1')); auto.
...
intros H'21; inversion H'21; red in |- *; auto.
apply appendfInclEq2; auto.
intros e H'15; rewrite H'20; auto.
apply H'14; auto.
Contradict H'15.
cut (InclEq _ eqRz L2 (appendf _ rZlt eqRz rZltEDec L2 L2')).
intros H'21; inversion H'21; red in |- *; auto.
...
intros f n1; elim n1; simpl in |- *; auto.
intros v Ar S H'; case (rArrayGet vM Ar v); auto.
intros H'0.
apply FStalCorrect0; auto.
...
case (doTripletsN getT Lt n1 Ar); auto.
intros Ar1 b1 L1 T1; case b1; auto.
intros H'0.
generalize (fun S : State => dilemma1RRCorrect f n1 Ar1 S H').
case (dilemma1RR f n1 Ar1); auto.
intros r b l t H'1.
apply FStalCorrectComp with (Ar' := Ar1); auto.
Qed.
(* We do the dilemma n times starting from v *)
...
intros L m n0; generalize L; elim n0; auto; clear L.
intros L Ar S;
 change (FStalCorrect Ar LL S (doTripletsN getT L m Ar)) in |- *.
apply doTripletFsNCorrect; auto.
intros n1 H' L Ar S0; simpl in |- *; auto.
apply dilemma1RRLCorrect; auto.
Qed.

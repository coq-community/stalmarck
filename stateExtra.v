...
simple destruct j; intros n Eq1 Eq2; unfold fnAux in |- *; simpl in |- *;
 ...
 (absurd (eqStateRz S (rZPlus n) i); auto; fail) ||
   absurd (eqStateRz S (rZMinus n) i); auto.
Qed.

Lemma negbElim : forall b b' : bool, negb b = negb b' -> b = b'.
simple destruct b; simple destruct b'; simpl in |- *; auto.
Qed.
Hint Immediate negbElim.
...
rewrite (RealizableAux2 S f i k); auto; rewrite (RealizableAux2 S f i j);
 auto.
red in |- *; intro; absurd (eqStateRz S k i); auto;
 apply eqStateRzTrans with (b := j); auto; apply eqStateRzSym; 
 auto.
red in |- *; intro; absurd (eqStateRz S (rZComp k) i); auto;
 apply eqStateRzTrans with (b := rZComp j); auto; apply eqStateRzSym; 
 auto.
Qed.
...
red in |- *; intros Abs1; absurd (contradictory (pair i j :: l)); auto.
unfold contradictory in |- *; exists i; auto;
 apply eqStateRzTrans with (b := rZComp j); auto.
red in |- *; unfold contradictory in |- *; intros Abs1; elim Abs1;
 intros a Pa.
absurd (contradictory (pair i j :: l)); auto; unfold contradictory in |- *;
 exists a; auto.
Qed.
...
exists f; auto.
exists (fun n : rNat => negb (f n)); split; auto with bool.
Qed.
...
exists (fnAux S f i); split; auto;
 ...
 auto.
rewrite Fij; case (rZEval f j); simpl in |- *; auto.
exists f; split; auto.
red in |- *; intros Contr; elim Contr; intros a Pa; absurd (eqStateRz S i j);
 auto; apply eqStateRzContr with (a := a); auto.
Qed.
